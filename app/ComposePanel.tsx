...
export default function ComposePanel({ project, graph, globalPeople, selectedPerson, onClose, onPreview, onApply, onAsk }: ComposePanelProps) {
  ...
  useEffect(() => {
    ...
    fetch("/api/compose", { headers: { accept: "application/json" }, cache: "no-store" })
      ...
      .then((payload: { available?: boolean; provider?: string }) => { if (active) { setProviderAvailable(Boolean(payload.available)); setProviderName(payload.provider ?? ""); } })
      .catch(() => { if (active) setProviderAvailable(false); });
    return () => { active = false; };
  }, []);
  ...
  async function createDraft(resolutions = semanticResolutions) {
    ...
    if (source === "describe" && providerAvailable !== true) { setStatus("Free-form Describe is not configured. Paste a list or import CSV instead."); return; }
    setLoading(true);
    ...
    try {
      if (source === "paste" || source === "csv") {
        ...
        const hydrated = { ...payload.draft, people: payload.draft.people.map((person) => { const global = globalPeople.find((item) => item.id === person.globalMatchId); return global ? { ...person, globalContact: { nickname: global.nickname, phone: global.phone, email: global.email, linkedinUrl: global.linkedinUrl, githubUrl: global.githubUrl } } : person; }) };
        setDraft(hydrated);
        setStatus(`${payload.draft.people.length} ${payload.draft.people.length === 1 ? "person" : "people"} found. Nothing has changed yet.`);
      }
    } catch (error) {
  ...
  return <aside ref={panelRef} tabIndex={-1} className="compose-panel" role="dialog" aria-modal="true" aria-labelledby="compose-title">
    ...
    {!draft ? <div className="compose-entry">
      ...
      {mode !== "ask" && <div className="compose-source-tabs">
        {(["describe", "paste", "csv"] as ComposeSource[]).map((item) => <button className={source === item ? "active" : ""} key={item} disabled={item === "describe" && providerAvailable === false} onClick={() => { setSource(item); setStatus(""); }}>{item === "paste" ? "Paste list" : item === "csv" ? "CSV" : "Describe"}</button>)}
      </div>}
      ...
      {source === "describe" && mode !== "ask" && <div className={`compose-provider-note ${providerAvailable === false ? "unavailable" : ""}`}>{providerAvailable === null ? "Preparing Circa’s language engine…" : providerAvailable ? `${providerName === "local-semantic" ? "Circa’s local semantic engine" : "Semantic language provider"} is ready. You will review every interpreted fact before anything changes.` : <><p>Free-form Describe is temporarily unavailable.</p><div><button onClick={() => { setSource("paste"); setStatus(""); }}>Use Paste list</button><button onClick={() => { setSource("csv"); setStatus(""); }}>Import CSV</button></div></>}</div>}
      {mode !== "ask" && <p className="compose-privacy">Your maps are stored in this browser. Local interpretation, Paste List and CSV stay on this device. If an external semantic provider is configured, Circa sends only the current Project’s minimum relevant graph context, never private notes or contact details.</p>}
      ...
      <button className="button button-dark compose-primary" onClick={() => void createDraft()} disabled={!text.trim() || loading || (source === "describe" && mode !== "ask" && providerAvailable !== true)}>{loading ? "Reading..." : copy.action} <span>→</span></button>
