...
| `users/{uid}/projectDeletionOperations/{projectId}` | Server-only deletion stage and completion receipt used to safely resume an owned-space deletion; contains IDs, mode, status and timestamps, not contact content | Privileged server only; Firebase/Firestore | Account lifetime; included in cloud export; deleted with account |
